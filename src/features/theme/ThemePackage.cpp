...
bool Package::load(Theme& theme,std::string& error) {
  entries_.clear();error="Invalid or truncated .stheme package";
  uint8_t b[12];uint32_t size=source_.size();
  ...
  for(unsigned i=0;i<count;++i) {
    ...
    if(e.path=="theme.json") {
      ...
      e.width=u16(b+4);e.height=u16(b+6);e.stride=b[8]?3:2;
    ...
    entries_.push_back(std::move(e));pos+=entries_.back().length;
  }
  if(pos!=size) return false;
  const Entry* manifest=find("theme.json");
  if(!manifest) {error="Missing theme.json";return false;}
  ...
  for(size_t i=0;i<candidate.layers.size();++i) {
    ...
    for(unsigned frame=0;frame<l.frames;++frame) {
      ...
      if(!e||!e->stride) {error=field+"missing asset: "+assetPath(l,frame);return false;}
      if(l.type==LayerType::Image) {l.width=e->width;l.height=e->height;}
      else if(l.width!=e->width||l.height!=e->height) {error=field+"frame dimensions do not match width/height: "+assetPath(l,frame);return false;}
    }
  }
  theme=std::move(candidate);error.clear();return true;
}
...
bool Package::row(AssetHandle handle,int y,int x,int count,uint16_t* colors,uint8_t* alpha) {
  ...
  for(int i=0;i<count;++i) {colors[i]=u16(bytes+i*e->stride);alpha[i]=e->stride==3?bytes[i*3+2]:255;}
  return true;
}
}
#endif
