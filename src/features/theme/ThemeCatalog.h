...
inline InstalledTheme inspectInstalledTheme(Source& source,const std::string& id) {
  ...
  entry.id=entry.name=id;entry.bytes=source.size();
  if(!validId(id)) {entry.error="Invalid installed filename";return entry;}
  Package package(source);Theme theme;
  if(!package.load(theme,entry.error)) return entry;
  if(theme.id!=id) {entry.error="Theme ID does not match installed filename";return entry;}
  entry.valid=true;entry.name=theme.name;entry.author=theme.author;entry.version=theme.version;
  return entry;
}
}
