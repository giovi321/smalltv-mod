...
int main() {
  JsonDocument config;config["wifi"]="working-network";config["hostname"]="smalltv-1234";
  ...
  assert(saveJsonAtomically(fs,config,"/config.json","/config.json.tmp"));
  JsonDocument saved;assert(!deserializeJson(saved,fs.files["/config.json"]));assert(saved["hostname"]=="smalltv-1234");assert(!fs.files.count("/config.json.tmp"));
